...
class FolderExpandableTable extends StatelessWidget {
 ...
 ExpandableTableRow _buildFolderRow(FolderModel folder, int depth) {
  ...
  // Ordenar archivos y subcarpetas para la recursividad
  ...
  return ExpandableTableRow(
   ...
   cells: [
    _buildFileCell('${folder.files.length} archivos • ${totalSizeMB.toStringAsFixed(2)} MB'),
    _buildFileCell('--'),                   
    _buildFileCell('--'),                   
    _buildFileCell('--'),                   
    ExpandableTableCell(               
 ...
 List<ExpandableTableRow> _buildRows() {
  // Ordenar las carpetas y archivos
  ...
  // Construir la lista combinada de filas
  final List<ExpandableTableRow> rows = [];
  rows.addAll(sortedFolders.map((f) => _buildFolderRow(f, 0)));
  rows.addAll(sortedFiles.map((f) => _buildFileRow(f, 0)));
  return rows;
 }
