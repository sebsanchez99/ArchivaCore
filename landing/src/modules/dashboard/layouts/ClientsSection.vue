<template>
  <div class="max-w-7xl mx-auto">
    ...
    <div v-else>
      ...
      <div class="overflow-x-auto bg-primary-100 rounded-lg shadow border border-base-content/5">
        <table class="table w-full">
          ...
          <tbody>
            <CompanyRow v-for="client in paginatedCompanies" :key="client.id" :client="client" @delete="openDeleteModal"
              @changePassword="openChangePasswordModal" @toggleStatus="openChangeStateModal"
      ...
      <div class="flex flex-col sm:flex-row justify-between items-center mt-4 gap-3">
        <span class="text-sm text-gray-500">
          Mostrando {{ startItem + 1 }}–{{ endItem }} de {{ filteredCompanies.length }} empresa{{ filteredCompanies.length
            === 1 ? '' : 's' }}
        </span>
    ...
    <!-- Modales -->
    <InfoModal ref="deleteModal" :title="`Eliminar empresa`"
      ...
      :onConfirmAction="handleDeleteCompany" :onCancelAction="handleCancel" />

    <InfoModal ref="changeStateModal" :title="`Cambiar estado de empresa`"
      ...
      :onConfirmAction="handleChangeStateCompany" :onCancelAction="handleCancel" />
...
function openCompanyLogs(company: Client) {
  selectedCompany.value = company
  companyLogDetails.value?.open(selectedCompany.value)
}
